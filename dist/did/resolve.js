 ...
 */
export async function resolveDidFromTx({ txid, client, factoryId = FACTORY_ALKANE_ID, pollIntervalMs = 2000, }) {
    // poll until the trace is ready
    let trace = null;
    for (;;) {
        try {
            trace = await client.getTrace({ vout: 4, txid });
            if (trace && trace.messages?.length)
                break;
        }
        catch (err) {
    ...
    const msg = trace.messages.find((m) => m.contractId === factoryId);
    if (!msg) {
        throw new Error(`Identity factory (contractId=${factoryId}) not found in trace for tx=${txid}`);
    }
    if (msg.status === "reject") {
        throw new Error(`Identity creation rejected. Reason: ${msg.error ?? "unknown"}`);
    }
